...
const ExamStudentsList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        if (!teacher?.teacher_id || !examId) return;

        const examResponse = await axios.get(
          `http://localhost:3301/auth/teacher/exams/${examId}/students`,
          {
                ...
                {students.map((student) => (
                  <tr key={student.attempt_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <Link
                        to={`/teacher/results/attempt/${student.attempt_id}`}
...
export default ExamStudentsList;
