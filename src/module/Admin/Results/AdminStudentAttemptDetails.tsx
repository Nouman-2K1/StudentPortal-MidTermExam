...
const AdminStudentAttemptDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError('');
        if (!admin?.token || !attemptId) return;

        const response = await axios.get(
          `http://localhost:3301/auth/admin/attempts/${attemptId}/details`,
          {
...
export default AdminStudentAttemptDetails;
